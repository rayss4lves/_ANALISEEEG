...
if __name__ == "__main__":
    ...
    # ==========================================================
    # Salvar/atualizar CSV com os tempos de execução
    # ==========================================================

    caminho_tempos = "meu_experimento_mfdfa/comparacao_tempos.csv"

    if os.path.exists(caminho_tempos):
        df_tempos = pd.read_csv(caminho_tempos)
    else:
        df_tempos = pd.DataFrame({
            "Execucao": np.arange(1, NUM_EXECUCOES + 1)
        })

    # Adiciona (ou substitui) as colunas dos arquivos executados
    for nome, tempos in resultados_tempos.items():
        df_tempos[nome] = tempos

    df_tempos.to_csv(caminho_tempos, index=False)

    # ==========================================================
    # Salvar/atualizar CSV com as estatísticas
    # ==========================================================

    caminho_est = "meu_experimento_mfdfa/estatisticas_tempos.csv"

    novas_est = pd.DataFrame({
    ...
    if os.path.exists(caminho_est):
        antigas_est = pd.read_csv(caminho_est)

        # Remove estatísticas antigas do mesmo arquivo
        antigas_est = antigas_est[
            ~antigas_est["Arquivo"].isin(novas_est["Arquivo"])
        ]

        df_estatisticas = pd.concat(
            [antigas_est, novas_est],
            ignore_index=True
        )
    else:
        df_estatisticas = novas_est

    df_estatisticas.to_csv(caminho_est, index=False)

    # ==========================================================
    # Gerar gráfico utilizando todos os arquivos presentes no CSV
    # ==========================================================

    df_plot = pd.read_csv(caminho_tempos)
    ...
    for coluna in df_plot.columns:
        ...
        plt.axhline(
            ...
            linewidth=1,
            alpha=0.8,
            label=f"Média {coluna}"
        )

    plt.title("Tempo de execução do MFDFA")
    plt.xlabel("Execução")
    plt.ylabel("Tempo (s)")
    plt.xticks(df_plot["Execucao"])
